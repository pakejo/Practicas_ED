...
void refleja_arbol(bintree<int> &arb)
{
  bintree<int> poda_izda, poda_dcha;

  //Podamos hijo derecho si existe
  if (!arb.root().right().null())
  {
    arb.prune_right(arb.root(), poda_dcha);
    refleja_arbol(poda_dcha);
  }

  // Podamos hijo izquierdo si existe
  if (!arb.root().left().null())
  {
    arb.prune_left(arb.root(), poda_izda);
    refleja_arbol(poda_izda);
  }

  arb.insert_left(arb.root(), poda_dcha);
  arb.insert_right(arb.root(), poda_izda);
}

void calcularPesoNodo(bintree<int>::node n, int &p)
{
  if(!n.parent().null())
    calcularPesoNodo(n.parent(),p+=*n);
}

void cuentaHojas(bintree<int>::node nodo, int &n)
{
  ...
  {
    if(!nodo.left().null())
      cuentaHojas(nodo.left(),n);

    if(!nodo.right().null())
      cuentaHojas(nodo.right(),n);
  }
...
{ // Creamos el árbol:
  ...
  bintree<int> arb2 = Arb;

  // 1) Procedimiento para reflejar un arbol
  refleja_arbol(arb2);
  Esquema(arb2, arb2.root(), pre);
  // 2) Funcion para calcular el peso de un nodo
  int peso = 0;
  calcularPesoNodo(Arb.root().right().left(),peso);
  cout <<"Peso del nodo: " << peso <<endl;
  // 3) Hacer un funcion de lectura en preorden con centinela

  // 4) Funcion que cuenta el numero de hojas
  int n_hojas = 0;
  cuentaHojas(Arb.root(),n_hojas);
  cout <<"Numero de hojas: " << n_hojas <<endl;
  // 5) Funcion que encuentre el numero de caminos cuya suma de etiquetas sea un valor k
